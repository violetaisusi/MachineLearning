 ...
 lstClass <- vector(mode="character",length=length(nmds))#check after using only one of lstClassAtt or lstClasAttE . May be we can delete if calculations are correct
  ...
  for(dsCount in 1:length(nmds)){        
    ...
    for(runCount in 1:runs){              
        ...
        print(paste("Run", runCount, sep=" -> "))
        
        print(paste("correctly Classified for this run",correctlyClassified,"of",length(predictions),sep=" -> "))        
        totalAccuracy <- totalAccuracy + (correctlyClassified/length(predictions))               
        
        AvgAccuracySub <- correctlyClassified / length(predictions)
        print(paste("correctly Classified Rate -> Accuracy rate for this run ->", AvgAccuracySub * 100, "%",sep=" "))
        
        AvgAUCRun <- (AUC/Number_of_Trees)
        print(paste("AUC rate for this run ->", AvgAUCRun * 100,"%", sep=" "))
        totalAUC <- totalAUC + AvgAUCRun
        print(paste("Entropy - Diversity rate for this run ->", mean(entropies) * 100,"%", sep=" -> "))        
        totalEntropy <- (totalEntropy + mean(entropies))        
    }#end for runcount      	  
    
    print(paste("Average Accuracy ->", (totalAccuracy/runCount) * 100,"%", sep=" "))
    print(paste("Average AUC ->", (totalAUC/runCount)*100,"%", sep=" "))
    print(paste("Average Entropy - Diversity ->", (totalEntropy/runCount) * 100,"%", sep=" "))
  }#end for datasetNo
