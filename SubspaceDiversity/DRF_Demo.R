main <- function(sepallength,sepalwidth,petallength,petalwidth,class){
 ...
 pathdsNewData <- c("arff-files/ds-ts/irisNewData.arff")
 ...
 lstClass <- vector(mode="character",length=length(nmds))#check after using only one of lstClassAtt or lstClasAttE . May be we can delete if calculations are correct
  ...
  for(dsCount in 1:length(nmds)){        
    ...
    dfTSTableA <- F_readDS(pathdsTS,TRUE,"\n","")
    #Add new data in  testing file #numAttTR is the same in training and testing
    F_newDataFile(nmds[dsCount],pathdsTS,dfTSTableA,pathdsNewData,numAttTR,sepallength,sepalwidth,petallength,petalwidth,class)  
    ...
    votes <- vector(mode="numeric",length=3) #position 1 Iris-setosa, 2 Iris-versicolor, 3 Iris-virginica 
        ...
        newPredicted <- vector(mode="character",length=Number_of_Trees)
        ...
        for(s in 0:limitTrees){#Number_of_Trees 500
          ...
          if(((s%%treesPerSub)==0) & (subspaceCount<Subspaces)){#treesPerSub change #even position in java because index starts in 0 - odd position in R because index of applicationArgs starts in 1      		                          
            ...
            dfNewData <- read.arff(pathdsNewData)
          ...
          ##New Data prediction
          newDatapredictions <- predict(model,newdata=dfNewData)  	
          
          newPredicted[(s+1)] <- as.character(newDatapredictions[1])#there is only one row in new data file 
          
          #print(paste("New prediction ",as.character(newDatapredictions[1]),sep=" "))
          ...
          for(j in 1:(length(predictions))){		                           
            ...
            #print(paste("actual ",actual,"predicted",predicted,sep="->"))             
        ...
        for(i in 1:Subspaces){            
          for(s in 1:Number_of_Trees){                                     
            #votingweightSub
            #print(paste("newPredicted ",newPredicted[s],"votes 1 ",votes[1]," votes 2 ",votes[2]," votes 3 ",votes[3],sep=" ->"))
            if(newPredicted[s]=="Iris-setosa"){
              votes[1] <- votes[1]+ votingWeightSub[i]
            }else if(newPredicted[s]=="Iris-versicolor"){
              votes[2] <- votes[2]+ votingWeightSub[i]              
            }else if(newPredicted[s]=="Iris-virginica"){
              votes[3] <- votes[3]+ votingWeightSub[i]              
            }
        ...
        #for(i in 1:3)
          #print(paste("vote ",i,votes[i],sep=" "))
        
        if((votes[1]>votes[2]) & (votes[1]>votes[3]))
           print(paste("The prediction for the class value is Iris-setosa"))
        else if((votes[2]>votes[1]) & (votes[2]>votes[3]))
          print(paste("The prediction for the class value is Iris-versicolor"))            
        else if((votes[3]>votes[1]) & (votes[3]>votes[2]))
          print(paste("The prediction for the class value is Iris-virginica"))             
...
#Add new data in testing file      
F_newDataFile <- function(nmds,pathdsTS,dfTSTableA,pathdsNewData,numAttTR,sepallength,sepalwidth,petallength,petalwidth,class){
  
  dfNewData <- data.frame()#dataframe to store info of testing subspaces files before write in file.     
  dfNewData <- rbind(dfNewData,data.frame(info=paste("@relation ",nmds,sep="")))  
  F_fileCreate(pathdsNewData)    
   
  for (k in 1:numAttTR){	               
    dfNewData <- rbind(dfNewData,data.frame(info=as.character(dfTSTableA[k,])))            
  }#end for k   
  
  dfNewData <- rbind(dfNewData,data.frame(info=paste("@data")))    
  
  #we have to put some value in the class but later model dont use it for predictions  
  dfNewData <- rbind(dfNewData,data.frame(info=paste(sepallength,",",sepalwidth,",",petallength,",",petalwidth,",",class,sep=""))) 
      
  F_writeTable(dfNewData,pathdsNewData ,"\n",TRUE,FALSE,FALSE,FALSE)  
}
